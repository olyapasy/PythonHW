...
student_name = input("Enter student’s name: (example: Mark Zuckerberg)")
name_split = student_name.split(" ")
name = name_split[0]
surname = name_split[1]
print("Transformation complete:")
print("%s  -> %s %s" % (student_name, surname, name))
...
info_lst = info.split('*')
persons_name = info_lst[0]
birth_date = info_lst[1]
death_date = info_lst[2]
birth_date_lst = birth_date.split("-")
death_date_lst = death_date.split("-")
birth_year = int(birth_date_lst[0])
death_year = int(death_date_lst[0])
